  ...
  <main>
    <header class="bg-blue-500 p-4">
      <select v-model="country" class="bg-white text-xs border border-gray-200 rounded p-2 float-right">
        <option v-for="country in countries" :value="country" :key="country.name">{{ country.code }}
    ...
    <router-view />
    <button @click="gotoCart" title="Cart" v-if="cartQuantity > 0 && currentRoute !== 'cart'"
  ...
  watch: {
    country: {
      handler(newval, oldval) {
        ...
        this.changeVatRate(newval.code);
        if (oldval) {
          this.changeExchangeRate({
            from: oldval.currency,
            to: newval.currency
          });
        }
      },
