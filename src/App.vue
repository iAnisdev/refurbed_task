<template>
  <main>
    <header class="bg-blue-500 p-4">
      <select v-model="country" class="bg-white text-xs border border-gray-200 rounded p-2 float-right">
        <option v-for="country in countries" :value="country" :key="country.name">{{country.code}} ({{country.currency}})</option>
      </select>
      <img class="w-32"
    ...
    <div class="grid grid-cols-2 gap-4 p-4 mb-4">
      <div class="mt-8 grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8">
       <Product v-for="product in products" :key="product.id" :product="product" :currency="country.sign" />
      </div>
    </div>
    <Cart :currency="country.sign" :vat="vat_rate"/>
  </main>
</template>
